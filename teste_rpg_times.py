...
for j in range(casos_teste):
    ...
    while True:
        ...
        for atacante in range(num_personagens):
            if personagens[atacante]['vivo']:
                alvo = alvos(atacante)
                personagens[alvo]['PV'] -= golpe(personagens[atacante], personagens[alvo])
                if personagens[alvo]['PV'] <= 0:
                    #vencedor = personagens[atacante]['nome']
                    #personagens[atacante]['vitorias']+=1
                    personagens[alvo]['PV'] = 0
                    personagens[atacante]['eliminacoes'] += 1
                    ...
                    if texto: print(personagens[alvo]['nome']+' morreu para '+personagens[atacante]['nome'])
                    #break;
        if texto:
